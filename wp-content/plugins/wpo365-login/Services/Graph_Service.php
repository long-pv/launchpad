	...
	class Graph_Service {
		...
		public static function fetch( $query, $method = 'GET', $binary = false, $headers = array(), $use_delegated = false, $prefetch = false, $post_fields = '', $scope = 'https://graph.microsoft.com/user.read', $log_level = 'WARN' ) {
			...
			if ( $multi_tenanted ) {
				...
				if ( $oidc_user_is_logging_in ) {
					$home_tenant_id = Options_Service::get_aad_option( 'tenant_id' );
					$user_tenant_id = ! empty( $id_token->tid ) ? $id_token->tid : '';

					if ( ! empty( $user_tenant_id ) && strcasecmp( $home_tenant_id, $user_tenant_id ) !== 0 ) {
						$use_delegated = true;
					}
				}
			}
			...
			$use_app_only              = Options_Service::get_aad_option( 'use_app_only_token', true ) || ( $use_mail_config );
			$no_sso                    = Options_Service::get_global_boolean_var( 'no_sso' );
			$uses_me                   = WordPress_Helpers::stripos( $query, '/me/' ) !== false;
			...
			if ( ! empty( $warning ) ) {
				Log_Service::write_log( $log_level, __METHOD__ . ' -> ' . $warning );
		...
		public static function is_fetch_result_ok( $fetch_result, $message, $level = 'ERROR', $on_error = null ) {
			...
			if ( $fetch_result['response_code'] < 200 || $fetch_result['response_code'] > 299 ) {
				...
				if ( is_array( $fetch_result ) && isset( $fetch_result['payload'] ) && isset( $fetch_result['payload']['error'] ) && isset( $fetch_result['payload']['error']['message'] ) ) {
					$error_code  = isset( $fetch_result['payload']['error']['code'] ) ? sprintf( ' | %s', $fetch_result['payload']['error']['code'] ) : '';
					$log_message = $message . ' [Error: ' . $fetch_result['payload']['error']['message'] . $error_code . ' ]';
					Log_Service::write_log( $level, __METHOD__ . ' -> ' . $log_message );
				...
				$output_log_level = $level === 'ERROR' ? 'WARN' : 'DEBUG';
				Log_Service::write_log( $output_log_level, $fetch_result );
				return false;
			}
